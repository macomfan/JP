...
public class FragmentRemember extends com.jp.ma.baseui.FragmentRemember {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        ...
        dict_ = DB.getInstance().getDatabase();
        currentWord_ = DB.getInstance().getWordSequence().current();
        displayWord();
        refreshFilterDisplay();
        return v;
    ...
    protected void onImgMyWordClicked(View v) {
        ...
        currentWord_.setTag(ITag.TAG_MY, "Y");
    ...
    private void refreshFilterDisplay() {
        ...
        for (Filter.FilterStruct f : filter.getCurrentFilter()) {
            ...
            if (entity != null) {
                mTxtFilterList.get(i).setText(entity.mShortname);
            } else {
    ...
    private void displayWord() {
        ...
        if (currentWord_.getTagValue(ITag.TAG_Skill).equals("")) {
            txtSkill_.setText("?");
        }
        else {
        ...
        txtRD_.setText(currentWord_.getTagValue(ITag.TAG_RD));
        mTxtCls.setText(currentWord_.getTagValue(ITag.TAG_Cls));

        if (!currentWord_.getTagValue(ITag.TAG_MY).equals("")) {
            mImgMyWord.setClicked(true);
        } else {
            mImgMyWord.setClicked(false);
        }
